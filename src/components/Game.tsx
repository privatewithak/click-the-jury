...
import type { LevelItem } from './levels.ts'
...
function Game() {
  ...
  const [levels, setLevels] = useState<LevelItem[]>(level)
  const [selected, setSelected] = useState<string>('card') // for shop selection

  const [isCrit, setIsCrit] = useState(false);
  const [critRemainingMs, setCritRemainingMs] = useState(0);

  const critTimeoutRef = useRef<number | null>(null);
  const critIntervalRef = useRef<number | null>(null);
